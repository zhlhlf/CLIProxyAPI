...
// When embedded mode is enabled (static/management.html exists), returns empty string
// to indicate the embedded asset should be used instead of file system.
func FilePath(configFilePath string) string {
	// Check if embedded asset is available
	if len(embeddedManagementHTML) > 0 {
		return "" // Return empty to indicate embedded mode
	}
...
func EnsureLatestManagementHTML(ctx context.Context, staticDir string, proxyURL string, panelRepository string) bool {
	// If embedded asset is available, always return true without file system operations
	if len(embeddedManagementHTML) > 0 {
		log.Debug("management asset is embedded, using in-memory version")
		return true
	}
...
// GetEmbeddedHTML returns the embedded management.html content if available.
// Returns empty slice if no embedded content is available.
